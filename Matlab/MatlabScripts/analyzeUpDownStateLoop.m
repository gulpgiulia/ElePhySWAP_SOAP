% analyzeUpDownStateLoop.m
%
%   Copyright 2018 Giulia De Bonis @INFN, Rome - Italy
%   Version: 1.0 - Aug. 01, 2018
%   revised version of the script, to be processed in a loop

%% 
fprintf(fid,['mouse:' mouseG '\n']);
fprintf(fid,['emission:' emissionG '\n']);
...
%
MUAdir = [BaseDirG 'RESULTS/Nov2016/' emissionG '/' mouseG '/' mouseG '_MUAdir/' FileName '/'];
...
%
if strcmpi(emissionG,'Evoked') 
   Stim = getSONEventData(hSMR, EventArray.port, Options.PeriodToAnalyze, fid);
end
...
for crsRecSet = ChannelSet % modified for running on a sub-set of recording channels (a cortical area)
   %%
   disp(['--- ' num2str(crsRecSet)]);
   close('all') % close figures of the previous channel
   clear ndx
   %% Make the working directory and change the current dir to it...
   %
   ...
   cd(tempdir);
   fprintf(fid,'----------------------------------------\n');
   fprintf(fid,['Recording set: ' RecordingSet(crsRecSet).label '\n']);

   %% Load dataset...
   %
   RawSig = getSONAnalogData(hSMR, RecordingSet(crsRecSet).port, Options.PeriodToAnalyze,fid);
   RawSig.dt = diff(RawSig.time(1:2));
   fprintf(fid,'Analyzed period: [%g,%g] s\n', RawSig.time(1), RawSig.time(end));  
...
%              if j~=1; A(j)=A(j)+A(j-1);end; %(here the check is not needed, if here, j cannot be = 1)
               ...
               fprintf(fid,'WARNING: logMUA below THR till the end of the recording time\n'); %break;
           end
       else; newloop = 0; 
   ...
   else; fprintf(fid,'WARNING: odd number of discontinuity borders\n');
   end
   
   if ~isempty(CutInterval)
       fprintf(fid,'DISCONTINUITY FOUND -- time interval(s)\n'); Disc=[Disc crsRecSet]; 
       fprintf(fid,'%f %f\n', [CutInterval]);
       %disp(CutInterval)
   end
...
%    rsMUA=test;
   ...
   if isfield(RecordingSet(crsRecSet),'HPFCutOffFreq')
      if ~isempty(RecordingSet(crsRecSet).HPFCutOffFreq)
         fprintf(fid,'LFP high-pass filtering...\n');
...
% SecondPeakArea:
   ...
   clear('ModeParams');
   [ModeParams, THR_2ndPeak,~,~,F1] = plotBimodalHistogram(rsMUA.value,fid);
   %close(gcf);
...
%[GDB] there should be added a check if the fit procedure is not converging...
   ...
   % --> check the RightPeak
   if ModeParams.FirstLeft==0; % the first (i.e. largest) peak is on the RIGHT)
...
% %    end
   ...
   fprintf(fid,'--- SHIFT logMUA and re-fit...\n');
   [ModeParams,~,hMUA,hX,F1] = plotBimodalHistogram(rsMUA.value,fid);
...
% % %     end
   ...
   %%%disp(ModeParams);
   fprintf(fid,'\nModeParams:\n');
   ff=fields(ModeParams);
   for k=1:numel(ff)
       fprintf(fid,'%15s: ',ff{k});
       if ~isstruct(getfield(ModeParams,ff{k}))
           fprintf(fid,'%s\n',num2str(getfield(ModeParams,ff{k})));
       else          
           fff=fields(getfield(ModeParams,ff{k}));
           for kk=1:numel(fff)
               fprintf(fid,'\n%22s: %s',fff{kk},num2str(getfield(ModeParams,ff{k},fff{kk})));
           end
           fprintf(fid,'\n');
       end
   end
      
% %    fprintf(fid,'Total Distribution:\n')
% %    disp(ModeParams.SkewnessTot);
% %    fprintf(fid,'Tail Distribution:\n')
% %    disp(ModeParams.SkewnessTail);
   
   if ModeParams.SecondPeakArea<THR_2ndPeak
       %fprintf('--- WeakBimodality (2nd peak is NOT FITTED)\n')
       fprintf(fid,'--- WeakBimodality\n');
       WeakBi=[WeakBi crsRecSet];
   end
   
   if (ModeParams.SkewnessTail.gamma)>1
       fprintf(fid,'--- [tail] StrongAsymmetry (positive)\n');
       SkewP=[SkewP crsRecSet];
   elseif (ModeParams.SkewnessTail.gamma)<-1
       fprintf(fid,'--- [tail] StrongAsymmetry (negative)\n');
...
% % %    end
   ...
   if ModeParams.FirstLeft
       UD_THRESHOLD = 2.*ModeParams.Sigma1;   
   else
       UD_THRESHOLD = 2.*ModeParams.Sigma2;
   end
   
   if(UD_THRESHOLD>ModeParams.SkewnessTail.MU) 
       fprintf(fid,'WARNING. UD_THRESHOLD is larger than Tail.MU!\n');
       LargeUDTH=[LargeUDTH crsRecSet];
   end
   ...
   if ModeParams.FirstLeft
       if(abs(2*ModeParams.Sigma1-UD_THRESHOLD)>0.05) OR (abs(ModeParams.Mu1)>0.05)
           ...
           fprintf(fid,'WARNING - check the results of the fit (Modeparams)\n');
...
% %    fprintf(fid,'\n');
% %    fprintf(fid,'histo_TH = %f\n', hX(find(hX>=UD_THRESHOLD,1)));
% %    fprintf(fid,'~~~ estimate of the percentage in the UP state: %f\n',UPpercent); 
% %    fprintf(fid,'\n');
...
%%% MIN_STATE_LEN = someUDcycle * RecordingSet(crsRecSet).UDcycleFraction;
   ...
   if ~isnan(UD_THRESHOLD)
      ...
      if length(Trans.val)<3 % at least 3 Trans for a valid evaluation of the states duration
          NoTrans=1;
          fprintf(fid,['WARNING: ' num2str(length(Trans.val)) ' transition(s) found (nTrans < 3)\n']);
          fprintf(fid,'... it is not possibile to evaluate the duration of Up and Down states.\n');
      ...
      if NoTrans~=1
          ...
          if ~isempty(CutInterval)
            for idx = 1:1:size(CutInterval,2)
                ...
                if ~isempty(F); fprintf(fid,'WARNING: Transitions found in the removed intervals\n'); end
            end
          end
          ...
          fprintf(fid,'... nTrans = %d\n',UDcycle.numel_duration);
          save('UpDownStateDuration', 'UpStateLen', 'DownStateLen', 'UDcycleLen');
          
          fprintf(fid,['<UD cycle> = ' num2str(mean(UpStateLen)+mean(DownStateLen),3) ...
...
if (numel(OUT_SD.low)~=0) 
    fprintf(fid,['(OUT_SD.low: ' num2str(OUT_SD.low) ')\n']);end
...
if (numel(OUT_SD.high)~=0)
    fprintf(fid,'WARNING: SD outlier channels\n');
    fprintf(fid,['(OUT_SD.high: ' num2str(OUT_SD.high) ')\n']);
...
if (numel(OUT_Ref.low)~=0) 
    fprintf(fid,['(OUT_LogMuaRef.low: ' num2str(OUT_Ref.low) ')\n']);end

OUT_Ref.high = find(LogMuaRef>Q3+ww);
if (numel(OUT_Ref.high)~=0)
    fprintf(fid,['(OUT_LogMuaRef.high: ' num2str(OUT_Ref.high) ')\n']);end
...
if (numel(OUT_F.high)~=0)
    fprintf(fid,['(OUT_F.high: ' num2str(OUT_F.high) ')\n']); end
OUT_F.low=find(Freq<Q1-ww);
if (numel(OUT_F.low)~=0)
    fprintf(fid,'WARNING: Frequency outlier channels\n');
    fprintf(fid,['(OUT_F.low: ' num2str(OUT_F.low) ')\n']); end
...
fprintf(fid,['...a total of ' num2str(numel(Disc)) ' channels with DISCONTINUITY\n']); 
if (numel(Disc))~=0; fprintf(fid,'%s\n',num2str(Disc)); end
%%% logMUA distribution
fprintf(fid,['...a total of ' num2str(numel(WeakBi)) ' channels with WeakBimodality\n']); 
if (numel(WeakBi))~=0; fprintf(fid,'%s\n',num2str(WeakBi)); end
fprintf(fid,['...a total of ' num2str(numel(RightPeak)) ' channels with the largest peak on the RIGHT (negative asymmetry)\n']); 
if (numel(RightPeak))~=0; fprintf(fid,'%s\n',num2str(RightPeak)); end
fprintf(fid,['...a total of ' num2str(numel(SkewP)) ' channels with positive StrongAsymmetry (second peak)\n']); 
if (numel(SkewP))~=0; fprintf(fid,'%s\n',num2str(SkewP)); end
fprintf(fid,['...a total of ' num2str(numel(SkewN)) ' channels with negative StrongAsymmetry (second peak)\n']); 
if (numel(SkewN))~=0; fprintf(fid,'%s\n',num2str(SkewN)); end
fprintf(fid,['...a total of ' num2str(numel(LargeUDTH)) ' channels with UD_THRESHOLD beyond the Tail.MU\n']); 
if (numel(LargeUDTH))~=0; fprintf(fid,'%s\n',num2str(LargeUDTH)); end
fprintf(fid,['...a total of ' num2str(numel(FitWarning)) ' channels with FitWarning (standardized Gaussian)\n']); 
if (numel(FitWarning))~=0; fprintf(fid,'%s\n',num2str(FitWarning)); end

fprintf(fid,['...a total of ' num2str(numel(FewTrans)) ' channels with FewTrans\n']); 
if (numel(FewTrans))~=0; fprintf(fid,'%s\n',num2str(FewTrans)); end

fprintf(fid,['...a total of ' num2str(numel(OUT)) ' outlier channels found\n']); 
if (numel(OUT))~=0; fprintf(fid,'%s\n',num2str(OUT)); end

catch
   msg = lasterror;
   fprintf(fid,'%s', msg.message);
   cd('..');
   if ~isempty(hSMR)
      closeSON(hSMR);
   end
